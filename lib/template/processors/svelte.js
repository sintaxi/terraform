    ...
    compile: function () {
      ...
      return function render(args) {
        ...
        if (hasProgessiveFlag) {
          ...
          // TODO: If we want to expose `current` etc. we need to run esbuild in render, not compile.
          var extraProps = {};
          ...
          var clientWrapper = `
            import Component from ${JSON.stringify(options.filename)}
          ...
          clientScript = esbuild.buildSync({
            stdin: {
              contents: clientWrapper,
              resolveDir: path.dirname(options.filename),
              sourcefile: path.basename(options.filename),
            },
