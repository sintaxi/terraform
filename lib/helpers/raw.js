...
var dataTree = exports.dataTree = function (filename) {
  ...
  try{
    var dataPath = path.resolve(dirPath, "_data.yaml")
    var fileData = fs.readFileSync(dataPath, 'utf8')
    var yamlDoc = yaml.safeLoad(fileData, {'strict': true});
    for (var attrname in yamlDoc) { obj._data[attrname] = yamlDoc[attrname]; }
  }catch(e){
