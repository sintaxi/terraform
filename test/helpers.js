...
describe("helpers", function(){
  ...
  describe('.priorityList(filename)', function(){
    ...
    it('should return all possible file names for html ordered by priority.', function(done){
      ...
      var plist = "index.pug, index.ejs, index.md, index.html.pug, index.html.ejs, index.html.md".split(', ')
    ...
    it('should build priority list assuming template file when unknown.', function(done){
      ...
      list.should.eql('feed.xml.pug, feed.xml.ejs, feed.xml.md'. split(', '))
      done()
    })
    ...
    it('should look for templates on json files.', function(done){
      ...
      list.should.be.an.instanceOf(Array)
      list.should.have.lengthOf(3)
      list.should.include('profile.json.pug')
      list.should.include('profile.json.ejs')
      list.should.include('profile.json.md')
      list.should.eql('profile.json.pug, profile.json.ejs, profile.json.md'. split(', '))
      done()
    })
    ...
    it('should look for templates when no ext present.', function(done){
      ...
      list.should.be.an.instanceOf(Array)
      list.should.have.lengthOf(3)
      list.should.eql('appcache.pug, appcache.ejs, appcache.md'.split(', '))
      done()
    })

    it('should know when filename is already correct.', function(done){
      polymer.helpers.buildPriorityList('index.pug').should.eql(['index.pug'])
      // TODO: Implement Me.
  ...
  describe('.findNearestLayout(root, filename)', function(){
    it('should find closest layout', function(done){
      ...
      polymer.helpers.findNearestLayout(root, "").should.eql("_layout.pug")
      polymer.helpers.findNearestLayout(root, null).should.eql("_layout.pug")
      done()
    })

    it('should find closest layout', function(done){
      var root = path.join(__dirname, 'fixtures', 'layouts', 'deep')
      polymer.helpers.findNearestLayout(root, "nested").should.eql(['nested', '_layout.pug'].join(path.sep))
      polymer.helpers.findNearestLayout(root, path.join("nested", "deeply")).should.eql(['nested', '_layout.pug'].join(path.sep))
      done()
    })
  })
  ...
  describe('.outputPath(filename)', function(){
    ...
    it('should allow alternate file extensions.', function(done){
      ...
      polymer.helpers.outputPath('foobar.bar.pug').should.eql('foobar.bar')
    ...
    it('should not allow alternate file extensions if disabled.', function(done){
      ...
      polymer.helpers.outputPath('foobar.bar.pug', false).should.eql('foobar.bar.html')
    ...
    it('should allow dot character on file name.', function(done){
      ...
      polymer.helpers.outputPath('foobar-1.0.0.pug').should.eql('foobar-1.0.0.html')
      polymer.helpers.outputPath('foobar-1.0.0.html.pug').should.eql('foobar-1.0.0.html')
    ...
    it('should allow dot character on file path.', function(done){
      ...
      polymer.helpers.outputPath('1.0.0/foobar.pug').should.eql('1.0.0/foobar.html')
      polymer.helpers.outputPath('1.0.0/foobar.html.pug').should.eql('1.0.0/foobar.html')
      done()
    })
  ...
  describe('.isTemplate(filename)', function(){
    ...
    it('should return true if pug file.', function(done){
      polymer.helpers.isTemplate(path.join('foo.pug')).should.be.true
      polymer.helpers.isTemplate(path.join('foo', 'bar', 'baz.pug')).should.be.true
      done()
    })
  ...
  describe('.isStylesheet(filename)', function(){
    ...
    it('should return false if pug file.', function(done){
      polymer.helpers.isStylesheet(path.join('foo.less')).should.be.true
      polymer.helpers.isStylesheet(path.join('foo', 'bar', 'baz.less')).should.be.true
      done()
    })
  ...
  describe('.getCurrent(sourcePath)', function(){
    ...
    it('should handle folders', function(done){
        polymer.helpers.getCurrent('to/some/file.json.pug').should.eql({
            'source': 'file.json',
    ...
    it('should handle folders', function(done){
        polymer.helpers.getCurrent('to/some/file.html.pug').should.eql({
            'source': 'file.html',
